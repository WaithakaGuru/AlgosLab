...
const Comparers = {
    MAX : (i: number, j: number) => i>j,
    MIN : (i: number, j: number) => i<j
}
...
export abstract class Heap extends HeapMethodsDeclarations {
    ...
    update(idx: number, val: number) {
        const comp = Comparers[this.type];
        if(val === this.heap[idx]) return
        else if(this.len() === 0) this.heap.push(val);
        else if(idx === 0 && comp(val, this.heap[0]) /*val < this.heap[0]*/) this.heap[0] = val;
        else if(idx === 0) {
            this.heap[idx] = val;
            this.siftDown(idx)
        }
    ...
    protected siftUp(i: number): void {     
        let parent = Math.floor((i-1)/2), comp = Comparers[this.type];
    ...
    protected siftDown(i: number): void {
        const opposite = this.type === "MAX"? "MIN" : "MAX", comp = Comparers[opposite];
